...
void YoloObjectDetector::init()
{
  ...
  boundingBoxesPublisher_ = nodeHandle_.advertise<darknet_msgs::bbox_array_stamped>("/yolo_bboxes", 1);
   ...
   cv::Scalar &rosBoxColor, const std::string &objectLabel)
{
  ...
  for (int i = 0; i < numberOfObjects; i++)
  {
     ...
     boundingBoxesResults_.bboxes.push_back(boundingBox);
...
void YoloObjectDetector::checkForObjectsActionGoalCB()
{
  ...
  boost::shared_ptr<const darknet_msgs::CheckForObjectsGoal> imageActionPtr = checkForObjectsActionServer_->acceptNewGoal();
...
bool YoloObjectDetector::publishDetectionImage(const cv::Mat& detectionImage)
{
  ...
  cvImage.header.frame_id = "sibot/camera_rgb_optical_frame";
